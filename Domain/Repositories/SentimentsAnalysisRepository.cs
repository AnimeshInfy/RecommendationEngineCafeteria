...
namespace Domain.Repositories
{
    public class SentimentsAnalysisRepository : ISentimentsAnalysisRepository
    {
        ...
        public async Task<string> CalcSentimentScores()
        {
            ...
            var goodSentiments = await _context.Sentiments.Where(x => x.Mood == "Positive").Select(x => x.Sentiments).ToListAsync();
            var badSentiments = await _context.Sentiments.Where(x => x.Mood == "Negative").Select(x => x.Sentiments).ToListAsync();
            ...
            await _context.SaveChangesAsync();
            await CalcAvgSentimentScore();
            return "Sentiment score calculated successfully";
        }
        private async Task<string> CalcAvgSentimentScore()
        {
            var feedbacks = await _context.Feedbacks.ToListAsync();
            var feedbackGroupByItemId = feedbacks.GroupBy(f => f.MenuItemId).Select(g => new
                                      { MenuItemId = g.Key,
                                        SentimentScoreAvg = g.Average(f => f.SentimentScore ?? 0)}).ToList();

            var menuItemIds = feedbackGroupByItemId.Select(f => f.MenuItemId).ToList();
            var menuItems = await _context.MenuItems.Where(mi => menuItemIds.Contains(mi.Id)).ToListAsync();

            foreach (var menuItem in menuItems)
            {
                var sentimentScoreAvg = feedbackGroupByItemId.FirstOrDefault(f => f.MenuItemId == menuItem.Id)?.SentimentScoreAvg;
